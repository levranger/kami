...
export function validateStep(step: BookingStep, state: {
  ...
  switch (step) {
    case 1:
      return validateConcerns(state.selectedConcerns);
    case 2:
      return validateGoal(state.selectedGoal);
    case 3:
      return validateEstimate(state.estimateAcknowledged);
    case 4:
      return validateContact(state.contactInfo);
    case 5:
      return validateDateTime(state.selectedDate, state.selectedTime);
    case 6:
...
function validateConcerns(concerns: string[]): ValidationResult {
  if (concerns.length === 0) {
    return { valid: false, errors: ["Please select at least one treatment concern."] };
  }
  return { valid: true, errors: [] };
}
...
function validateGoal(goal: TreatmentGoal | null): ValidationResult {
  if (!goal) {
    return { valid: false, errors: ["Please select a treatment goal."] };
  }
  return { valid: true, errors: [] };
}
...
function validateEstimate(acknowledged: boolean): ValidationResult {
  if (!acknowledged) {
    return { valid: false, errors: ["Please acknowledge that this is an estimate only."] };
  }
  return { valid: true, errors: [] };
}
...
function validateContact(contact: ContactInfo): ValidationResult {
  ...
  if (!emailRegex.test(contact.email.trim().toLowerCase())) {
