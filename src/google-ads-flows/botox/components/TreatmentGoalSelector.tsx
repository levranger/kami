...
import { goalOptions } from "../lib/estimates";
...
export function TreatmentGoalSelector({ selectedGoal, onSelect, onContinue, onBack, error }: TreatmentGoalSelectorProps) {
  return (
    <div>
      <h2 className="mb-2 text-xl font-bold text-slate-900">Treatment Goal</h2>
        ...
        {goalOptions.map((option) => {
          ...
          return (
            <button
              ...
              className={`flex min-h-[52px] w-full items-center gap-3 rounded-lg border-2 p-4 text-left transition-all ${
                isSelected
            ...
            >
              <div
                className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${
                  isSelected ? "bg-amber-600" : "border-2 border-slate-300"
                }`}
              >
      ...
      <p className="mt-4 text-xs text-slate-400">
        This preference helps us understand your goals. Your provider will determine whether treatment is appropriate and recommend the final dose.
      </p>
