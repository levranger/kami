...
interface BotoxPayload {
  ...
  selectedProductPreference: string;
  appointmentType: string;
  selectedDate: string;
  selectedTime: string;
  estimateSummary: {
    estimatedUnits?: { min: number; max: number } | null;
...
function clientHtml(p: BotoxPayload): string {
  const concerns = (p.concerns ?? p.selectedConcerns ?? []).join(", ") || "—";
...
<tr><td align="center"><table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fff;border-radius:4px;border:1px solid #e8e0d8;">
  ...
  <tr><td style="padding:36px 40px;">
    ...
    <p style="margin:0 0 24px;color:#3a3a3a;font-size:15px;line-height:1.6;">
      We've received your Botox appointment request. Our team will reach out shortly to confirm availability and next steps.
    </p>
    ...
    <tr><td style="padding:24px;">
      <p style="margin:0 0 4px;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#888;font-weight:600;">Treatment Concerns</p>
      <p style="margin:0 0 16px;font-size:14px;color:#1a1a1a;">${concerns}</p>
...
function staffHtml(p: BotoxPayload): string {
  const concerns = (p.concerns ?? p.selectedConcerns ?? []).join(", ") || "—";
...
<tr><td align="center"><table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fff;border-radius:4px;border:1px solid #ddd;">
  <tr><td style="background:#1a1a1a;padding:20px 32px;">
    ...
    <p style="margin:4px 0 0;color:#fff;font-size:18px;">Botox / Neurotoxin</p>
  </td></tr>
  <tr><td style="padding:28px 32px;">
    ...
    <p style="margin:0 0 4px;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#888;">Concerns</p>
    <p style="margin:0 0 16px;font-size:14px;color:#1a1a1a;">${concerns}</p>
    ...
    <p style="margin:0 0 4px;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#888;">Product Preference</p>
    <p style="margin:0 0 16px;font-size:14px;color:#1a1a1a;">${p.selectedProductPreference}</p>
    <p style="margin:0 0 4px;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#888;">Estimate</p>
    <p style="margin:0 0 4px;font-size:14px;color:#1a1a1a;">${formatRange(p.estimateSummary?.estimatedUnits)} units &mdash; ${formatRange(p.estimateSummary?.estimatedPrice, "$")}</p>
...
export async function POST(req: NextRequest) {
  try {
    const payload = (await req.json()) as BotoxPayload;

    const [clientResult, staffResult] = await Promise.allSettled([
      resend.emails.send({
        from: `Kami Aesthetics <${FROM}>`,
        to: payload.contactInfo.email,
        subject: "Your Botox Appointment Request — Kami Aesthetics",
        html: clientHtml(payload),
      }),
      resend.emails.send({
        from: `Kami Bookings <${FROM}>`,
        to: NOTIFY,
        subject: `New Botox Booking — ${payload.contactInfo.fullName} — ${payload.selectedDate}`,
        html: staffHtml(payload),
      }),
    ]);

    if (clientResult.status === "rejected") {
      console.error("[Botox API] Client email failed:", clientResult.reason);
    } else if (clientResult.value.error) {
      console.error("[Botox API] Client email Resend error:", clientResult.value.error);
    }

    if (staffResult.status === "rejected") {
      console.error("[Botox API] Staff email failed:", staffResult.reason);
    } else if (staffResult.value.error) {
      console.error("[Botox API] Staff email Resend error:", staffResult.value.error);
    }
